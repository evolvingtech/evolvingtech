...
A Lean-based contract algebra for architectural-level verification of how engineered components fit together. Four named operators (sequential composition, shared-resource composition, refinement, conformance) with axiom-free soundness theorems. Interfaces are first-class objects carrying explicit assumes, guarantees, and silences (aspects deliberately left unmodeled, surfaced as typed data rather than hidden). The Contract type is parameterized over a modality marker: v0.2.1 ships two modalities, the default Untimed and a Timed modality that carries per-component latency and composes it (summed under sequential composition, taken as the maximum under shared-resource composition). The timed modality was added with no change to the operators or their soundness theorems, demonstrating the substrate's design property that further modalities extend it through downstream typeclass instances rather than edits to the core.
